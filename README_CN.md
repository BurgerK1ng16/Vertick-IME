...
- 内置 `8105 + base + others` 预编译基础包，安装后直接加载，不在手机上编译词典。增强词典、英文、Emoji 与万象长句语法均由用户明确开启后下载；下载完成校验后才会原子切换，不会中断可用词典。
- 新增完全离线的联想词：仅在没有拼音或英文组合态时展示下一词建议。可选本机词对学习有数量上限并可一键清除，不保存完整输入历史、原始拼音、录音或云端文本。
...
- **听写**：调用 ASR 模型识别后，先按本地标点规则清洗，再一次性输出结果。
